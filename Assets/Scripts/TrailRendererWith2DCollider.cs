...
public class TrailRendererWith2DCollider : MonoBehaviour
{
    ...
    public bool isFriendly = true; //determines if the trail is pausing, i.e. neither creating nor destroying vertices
    ...
    private void Awake()
    {
        ...
        if(circleEnd != null)
        circleEnd.gameObject.SetActive(false);
    }
    ...
    private void Update()
    {
        ...
        if(centerPositions.Count >= 5 && circleEnd != null)
        {
            //Vector3 pos = leftVertices.Last.Value.Position;
            circleEnd.gameObject.SetActive(true);
            circleEnd.position = centerPositions.Last.Previous.Value;
            /*if (pos != null)
            {
